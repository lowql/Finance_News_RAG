from storages.build.property_graph import ManualBuildPropertyGraph,graph_store
import os
...
def test_set_summary_by_llm():
    ...
    cypher = """
    MATCH (n:`新聞`)-[:`提及`]->(c:`公司`)
    WHERE n.headline CONTAINS "焦點股" AND NOT n.author in ["時報資訊"]
    ...
    for row in rows:
        ...
        try:
            gen_summary(row['content'],mode='stream')
